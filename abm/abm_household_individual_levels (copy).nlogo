...
households-own [
  ...
  agriculture_labor_force
  land_abandoned
  land_growth_capacity
  life_cycle_stage
  hh_members
  workers
  nonwork
...
;Create agriculture clusters
 to setup_agent_on_grid
  ...
  ;lines below somewhere from the internet
   let horizontal-spacing (world-width / sqrt(household_number))
   let vertical-spacing (world-height / sqrt(household_number))
   let min-xpos (min-pxcor - 0.5 + horizontal-spacing / 2)
   let min-ypos (min-pycor - 0.5 + vertical-spacing / 2)
  nw:generate-lattice-2d households links sqrt(household_number) sqrt(household_number) false [
  let row (floor (who / sqrt(household_number)))
     let col (who mod sqrt(household_number))
...
to parametrize_agents
  file-close-all
  file-open "../mmp_data/household_sample.csv"
  let data csv:from-row file-read-line
  ask households [
    ;empirical components
    set data csv:from-row file-read-line
    set agriculture_land item 0 data + 1 ;tmp fix only for non-correct imported data
    set life_cycle_stage item 1 data
    set hh_members item 2 data
    set workers item 3 data
    set nonwork item 4 data
    set commun item 5 data
    set hhnum item 6 data
    set surveyyr item 7 data
    ;Indirectly derived attributes
    set agriculture_income agriculture_land
    set land_abandoned TRUE
    set migrated_hh_members 0
    set agriculture_labor_force 1
    set add_workers 0
...
set t_0_land_amount sum [agriculture_land] of households
  ...
  file-close-all
  file-open "../mmp_data/household_members_sample.csv"
  set data csv:from-row file-read-line
  ask individuals [
    set data csv:from-row file-read-line
...
to go
 ...
 if time_frame = "empirical_data" [
  set empirical_agriculture_pattern (list 0 0.016 0.032 0.049 0.06 0.076 0.08 0.09 0.1 0.11 0.11 0.11 0.12 0.11 0.11 0.1 0.1)
...
to individuals_demographics_module
  ...
  if member_sex = 1 AND member_age > 18 AND member_age < 40 AND random-float 1 < 0.05 AND member_migrated = FALSE [
...
to household_demographics_module
  ...
  let household_children count individuals with [ member_commun = household_commun AND member_hhnum = household_hhnum AND member_surveyyr = household_surveyyr  AND
  member_age < 13]

  let household_teenager  count individuals with [ member_commun = household_commun AND member_hhnum = household_hhnum AND member_surveyyr = household_surveyyr  AND
  member_age > 13 AND member_age < 18]

  let household_adults_non_head count individuals with [ member_commun = household_commun AND member_hhnum = household_hhnum AND member_surveyyr = household_surveyyr  AND
 member_age > 18 AND member_relhead  > 1 ]

  if household_adults_non_head = 0 AND household_teenager = 0 AND household_children = 0 [
  set life_cycle_stage 1 ]

  if household_adults_non_head = 0 AND household_teenager = 0 AND household_children > 0 [
  set life_cycle_stage 2 ]

  if household_adults_non_head = 0 AND household_teenager > 0  [
  set life_cycle_stage 3 ]


  if household_adults_non_head = 0 AND household_teenager > 0 AND household_children = 0 [
  set life_cycle_stage 4 ]


   if household_adults_non_head > 0 AND household_teenager = 0 AND household_children = 0 [
  set life_cycle_stage 5 ]
  ...
  ;create a new household structure or multiple ones when household head retires with 80 y/o
  ;get the age of household head
  ...
  print "------------------------------------"
  print who
  type "children "   type household_children print " "
  type "household_teenager "   type household_teenager print " "
...
to migration_decision
  if member_age > 18 AND member_migrated = FALSE [
...
let individual_utlitity_land_tmp 0
    ...
    if [ member_relhead ] of self = 3 [
      let relative_utility_factor count individuals with [member_commun =  member_commun_spec  AND member_hhnum = member_hhnum_spec AND member_surveyyr = member_surveyyr_spec AND member_migrated = FALSE] ;member_relhead = 3 AND]
...
to receiving_remittances
 if migrated_hh_members > 0 [
    ...
    foreach tmp_list [
      if random-float 1 < probability_receiving_remittances [
      set received_remittances received_remittances + 1
      ]
    ]

    if received_remittances > 0 [
    set tmp_list n-values received_remittances [1]
      foreach tmp_list [
        if random-float 1 < probability_remittances_agriculture_invest [
          set add_workers add_workers + 1
      ]
      ]
    ]
  ]
...
to agriculture_practices
  ...
  ;set agriculture_labor_force  (workers + add_workers)  / (workers + migrated_hh_members)
    ;farmland abandonment

  if deforestation_mode = "proportional" [

    if agriculture_labor_force < 1 AND land_abandoned = FALSE [
    set agriculture_land agriculture_land * agriculture_labor_force
    set global_abandoned_land global_abandoned_land + agriculture_land * agriculture_labor_force
    set land_abandoned TRUE
    ]
    ;case of (potential) expansion
    if agriculture_labor_force >= 1  AND land_abandoned = FALSE [
      set agriculture_land agriculture_land +   land_clearing_param * agriculture_land
  ]
  ]



   if deforestation_mode = "new_method" [



     ifelse  land_abandoned = FALSE [
      if workers > 0 [
        set agriculture_land agriculture_land - ((1 /  ([ workers ] of self)) *  [agriculture_land] of self )
        set global_abandoned_land global_abandoned_land + ((1 /  ([ workers ] of self)) *  [agriculture_land] of self )
...
to create_new_household

  ; select to individuals where model selects to be married, they die, get replaced by new individuals in spawning household
  ask one-of individuals with [married = TRUE AND member_sex = 1] [
  die ]
  ask one-of individuals with [married = TRUE AND member_sex = 2] [
  die ]

  set global_household_index global_household_index + 1
  create-households 1 [

  set commun 2021
  set hhnum global_household_index
  set surveyyr 2021
  set agriculture_land  random-float 10 ; needs to be edited to represent inheritance later on
  set life_cycle_stage 1
  set hh_members 2
  set workers 2
  set nonwork 0
  set agriculture_income agriculture_land
  set land_abandoned FALSE
  set migrated_hh_members 0
  set agriculture_labor_force 1
  set add_workers 0
  ...
  let hh_children random 5 + 1

  create-individuals 1 [
  set member_commun 2021
  set member_hhnum global_household_index
  set member_surveyyr 2021
  set member_relhead 0
  set member_age 30 ; needs to be adjusted later
  set member_sex  1
  set married FALSE
  set member_migrated FALSE
  ]
  ...
  set member_hhnum global_household_index
  set member_surveyyr 2021
  set member_relhead 1
  set member_age 30 ; needs to be adjusted later
  set member_sex  2
  set married TRUE ; needs to be set for relhead 1 or 2 in constructor of ABM too
  set member_migrated FALSE
  ]

      let tmp_list n-values hh_children [1]
      foreach tmp_list [
    create-individuals 1 [
  set member_commun 2021
  set member_hhnum global_household_index
...
to update_land_inequality_levels
  ...
  let median_land median [agriculture_land] of households
  ask  households [
    set agriculture_land median_land
  ]
end
...
324
569
probability_receiving_remittances
probability_receiving_remittances
0
1
0.7
...
"" ""
PENS
"default" 1.0 1 -16777216 true "" " histogram  [individual_utlitity_land] of individuals"
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="inital_hh_translocal">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="change_inequality">
      <value value="&quot;off&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="land_clearing_param">
      <value value="0.02"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="deforestation_mode">
      <value value="&quot;new_method&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="internal_validation_mode">
      <value value="&quot;off&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="household_number">
      <value value="225"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="scaling_param_migration">
      <value value="0.92"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="probability_receiving_remittances">
      <value value="0.7"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="deforestation_per_step_worker_ha">
      <value value="0.079"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="re_distribution_factor">
      <value value="1.4"/>
    </enumeratedValueSet>
